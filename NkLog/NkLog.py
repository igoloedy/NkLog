...
class NkLog() :
    ...
    def __init__(self,filename = '',format = '',level=10,stdout = False):
        ...
        #ハンドラを登録
        self.set_handler(filename,self.format,stdout)
    
    def set_handler(self,filename,format,fileout = False, stdout = True):
        '''
        ハンドラの登録
...
if __name__ == '__main__':
    mg = NkLog(filename="log\\NkLogMainFunc.log", stdout=True)
    mg.setLevel(WARN)
    mg.debug("debug")
    mg.warning("warn")
    mg.error("error")
